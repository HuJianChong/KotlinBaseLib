...
class AppUtils private constructor() {
    ...
    companion object {
        ...
        val versionCode: Int by lazy {
            Logger.d("get versionCode")
            var versionCode = 0
            try {
                val packageName = BaseApplication.context.packageName
                versionCode = BaseApplication.context.packageManager.getPackageInfo(packageName, 0).versionCode
        ...
        val versionName: String by lazy {
            Logger.d("get versionName")
            var versionName = ""
            try {
                val packageName = BaseApplication.context.packageName
                versionName = BaseApplication.context.packageManager.getPackageInfo(packageName, 0).versionName
            } catch (e: PackageManager.NameNotFoundException) {
                e.printStackTrace()
            }
            versionName
